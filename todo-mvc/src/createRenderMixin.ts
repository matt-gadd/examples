...
import { Parent, RenderFunction, StylesHash } from 'dojo-widgets/mixins/interfaces';
...
const childrenCache = new WeakMap<RenderMixin<RenderMixinState>, Map<any, any>>();

const newChildrenCache = new WeakMap<RenderMixin<RenderMixinState>, Map<any, any>>();
...
const createRenderMixin = createStateful
	.mixin<Render, RenderMixinOptions<RenderMixinState>>({
		...
		initialize(instance: RenderMixin<RenderMixinState>, options: RenderMixinOptions<RenderMixinState> = {}) {
			...
			childrenCache.set(instance, new Map());

			newChildrenCache.set(instance, new Map());
